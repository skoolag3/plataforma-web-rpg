...
export function analisarResposta(resposta: string): Dificuldade {
  ...
  const confianca = ['vou vencer', 'sem medo', 'derrotar', 'acabar com', 'pronto', 'desafio'];
...
export function iniciarBatalha(
  ...
  const montar = (cartas: CartaBatalhaBase[], multiplicador = 1) =>
    cartas.map((carta) => ({
      ...
      velocidadeAtual: Math.max(1, Math.round(carta.velocidade * multiplicador)),
      derrotada: false,
    }));
  const estado: EstadoBatalha = {
    ...
    bot: { cartas: montar(bot, multiplicadorDificuldade[dificuldade]), ativa: 0 },
    eventos: [],
  };
...
export function executarTurno(estado: EstadoBatalha): EstadoBatalha {
  ...
  estado.turno += 1;
  const jogador = cartaAtiva(estado, 'JOGADOR');
  const bot = cartaAtiva(estado, 'BOT');
  const ordem: Lado[] =
    jogador.velocidadeAtual >= bot.velocidadeAtual
      ? ['JOGADOR', 'BOT']
      : ['BOT', 'JOGADOR'];
...
function atacar(estado: EstadoBatalha, origem: Lado, alvo: Lado) {
  ...
  const dano = Math.max(1, Math.round(atacante.ataqueAtual - defensor.defesaAtual * 0.45));
  defensor.hpAtual = Math.max(0, defensor.hpAtual - dano);
  estado.eventos.push(
    { turno: estado.turno, tipo: 'ATAQUE', origem, texto: `${atacante.nome} atacou ${defensor.nome}.` },
    { turno: estado.turno, tipo: 'DANO', origem, valor: dano, texto: `${defensor.nome} recebeu ${dano} de dano.` },
  );
...
function derrotar(estado: EstadoBatalha, lado: Lado) {
  ...
  estado.eventos.push({ turno: estado.turno, tipo: 'DERROTA', texto: `${derrotada.nome} foi derrotada.` });
  const proxima = equipe.cartas.findIndex((carta, indice) => indice > equipe.ativa && !carta.derrotada);
  if (proxima < 0) {
    estado.status = 'FINALIZADA';
    estado.vencedor = lado === 'JOGADOR' ? 'BOT' : 'JOGADOR';
    estado.eventos.push({ turno: estado.turno, tipo: 'FIM', texto: `${estado.vencedor} venceu a partida.` });
  } else {
...
function aplicarEntrada(estado: EstadoBatalha, lado: Lado) {
  ...
  estado.eventos.push({ turno: estado.turno, tipo: 'ENTRADA', origem: lado, texto: `${carta.nome} entrou em campo.` });
  const passiva = carta.passiva ?? {};
  if (passiva.gatilho !== 'on_enter') return;
  const alvo = passiva.alvo === 'enemy' ? (lado === 'JOGADOR' ? 'BOT' : 'JOGADOR') : lado;
...
function aplicarPassivaAtaque(estado: EstadoBatalha, lado: Lado, alvo: Lado) {
  ...
  if (passiva.gatilho === 'on_attack') aplicarModificador(estado, lado, passiva.alvo === 'enemy' ? alvo : lado, passiva);
}

function aplicarModificador(estado: EstadoBatalha, origem: Lado, alvo: Lado, passiva: Record<string, unknown>) {
  ...
  const atributo = passiva.atributo === 'defesa' || passiva.atributo === 'velocidade' ? passiva.atributo : 'ataque';
  const chave = `${atributo}Atual` as 'ataqueAtual' | 'defesaAtual' | 'velocidadeAtual';
  carta[chave] = Math.max(1, carta[chave] + fator * valor);
  const tipo = fator > 0 ? 'BUFF' : 'DEBUFF';
  estado.eventos.push({ turno: estado.turno, tipo, origem, valor, texto: `${carta.nome}: ${atributo} ${fator > 0 ? '+' : '-'}${valor}.` });
}
